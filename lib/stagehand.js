...
var Stagehand = function(callback){
      ...
      if(exists){
        ...
        ncp(process.cwd() + '/tmp/' + name, process.cwd() + '/tmp/mocks/' + name, function(err){
          if(err){
            ...
            rewriteConfigurationSettings(name, dir, options, false, function(rewrites){
              callback(rewrites);
            });
          }
        });
      ...
      if(exists){
        ...
        ncp(process.cwd() + '/tmp/' + name, process.cwd() + '/tmp/isolated/' + name, function(err){
          if(err){
            ...
            // Rewrite any outbound http traffic in the configs so that they go to our proxy
            rewriteConfigurationSettings(name, dir, options, true, function(rewrites){
              callback(rewrites);
            });
          }
        });
  ...
  // ------------------------------------------------------------------------------------------------
  var rewriteConfigurationSettings = function(name, dir, options, isolation, callback){
    var ret = {},
        _self = this;
          ...
          if(err){
            ...
            files.forEach(function(file){      
              ...
              if((file.indexOf('.yaml') > 0 || file.indexOf('.yml') > 0) && file.indexOf('.bak') < 0 && file.indexOf('.example') < 0){
								...
								var rewrite_rules = (isolation ? options['rewrite_lines_for_isolation'] : options['rewrite_lines_for_mocking'])
                ...
                fs.stat(dir + '/' + file, function(err, stats){
                  if(stats.isDirectory()){
                    rewriteConfigurationSettings(name, dir + '/' + file, options, isolation, function(rewrites){
