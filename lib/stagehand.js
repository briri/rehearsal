...
var Stagehand = function(callback){
      ...
      if(exists){
        ...
        ncp(process.cwd() + '/tmp/' + name, process.cwd() + '/tmp/mocks/' + name, function(err){
          if(err){
            ...
            rewriteConfigurationSettings(name, dir, function(success){
              callback(success);
            });
          }
        });
  ...
  // ------------------------------------------------------------------------------------------------
  var rewriteConfigurationSettings = function(name, dir, callback){
    var ret = [],
				_self = this,
        proxy_uri = 'http://localhost:9099';
              ...
              if((file.indexOf('.yaml') > 0 || file.indexOf('.yml') > 0) && file.indexOf('.bak') < 0 && file.indexOf('.example') < 0){
                      ...
                      if(data){
                        fs.writeFile(dir + '/' + file, 
                            ...
                            fs.readFile(dir + '/' + file, function(err, data){
                            	if(err) console.log('Error reading config file: ' + err);
                
                            	_.forEach(data.toString().match(/http:\/\/localhost:9099\/[\w\/]+/g), function(uri){
																ret.push(uri);
                            	});
                            });
                ...
                fs.stat(dir + '/' + file, function(err, stats){
                  if(stats.isDirectory()){
                    rewriteConfigurationSettings(name, dir + '/' + file, function(uris){
          ...
          if(success){ 
            ...
            buildSet(name, def, function(success){
              if(success){ 
                ...
                buildMockSet(name, def, function(uris){
